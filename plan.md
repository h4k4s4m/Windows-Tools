# ROBOCOPY GUI Implementation Plan

## Overview
A PowerShell GUI tool to provide a user-friendly interface for the Robocopy command.

## Features
- browse buttons for source and destination directories
- tabs for options: copy options, file selection options, retry options, logging options, job options
- button to run or copy command to clipboard
- save configuration
- load configuration
- clear configuration (to defaults)
- help button to show Robocopy help
- hover tooltips for each option
- each option should also have a short description
- almost every option should be implemented except for the most advanced and technical ones (throttling, offload, compression, etc.)
- defaults already checked and filled out (use refernce)

## implementation
- use WPF for GUI
- use PowerShell for backend (make compatible with PowerShell 5.1)
- use Robocopy command for file copying
- use Robocopy help for tooltips and help button
- json for saving and loading configuration

## notes
- do not use Robocopy.exe, use the Robocopy command in PowerShell
- write the program in logical chunks, make tabs first, fill out the contents of a tab, then move to the next tab
- if possible, make the layout responsive to window size




refernce help page for robocopy:

   ROBOCOPY     ::     Robust File Copy for Windows
-------------------------------------------------------------------------------

  Started : Monday, January 13, 2025 9:07:11 PM
